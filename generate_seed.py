...
def is_node_occupied(node_name, all_vehicles, this_vehicle):
    # If some other vehicle is *exactly* on that node, consider it occupied.
...
### CHANGES: Remove all traffic-light classes and references. ###
# (We are no longer controlling or checking any signals.)
...
class ExtremeHeatEffect(WeatherEffect):
    def __init__(self, screen_width, screen_height):
        self.screen_width = screen_width
        self.screen_height = screen_height
        # Overlay to simulate bright "heat haze"
...
class ModerateWindEffect(WeatherEffect):
    ...
    def update(self, dt):
        for line in self.lines:
            # Move each streak horizontally
            line[0] += line[3]
            # If off the screen to the right, reset to left
...
class BaseVehicle:
    ...
    def __init__(self, graph, start_node, side, speed=2):
        ...
        self.num_nodes_passed = 0
        self.vehicle_type = "Generic"
        self.nodes_passed_list = [start_node]  # track node path
    ...
    def move(self, all_vehicles):
        ...
        if dist < current_speed:
            ...
            self.x = target_x
            self.y = target_y
            self.nodes_passed_list.append(self.current_node)
            ...
            # Move fractionally towards the node
            if current_speed > 0:
...
class Motorcycle(BaseVehicle):
    def __init__(self, images, graph, start_node, side, speed=1.75):
        super().__init__(graph, start_node, side, speed)
...
class Bulldozer(BaseVehicle):
    def __init__(self, images, graph, start_node, side, speed=0.8):
        super().__init__(graph, start_node, side, speed)
...
def spawn_vehicles_thread(
    ...
    while simulation_running:
        ...
        # Weighted random choice for each side (Tier 2 logic)
        ...
        # Tier 3 speed overrides
        speed_s = tier3_speeds.get(vehicle_type_s, 1.0)
        speed_m = tier3_speeds.get(vehicle_type_m, 1.0)

        with lock:
            # Create static-side vehicle
            if vehicle_type_s == "car":
                new_vehicle_s = Car(cars_img, graph_static, entry_s, side="static", speed=speed_s)
            elif vehicle_type_s == "truck":
                new_vehicle_s = Truck(trucks_img, graph_static, entry_s, side="static", speed=speed_s)
            elif vehicle_type_s == "motorcycle":
                new_vehicle_s = Motorcycle(motos_img, graph_static, entry_s, side="static", speed=speed_s)
            elif vehicle_type_s == "schoolbus":
                new_vehicle_s = Schoolbus(schoolbus_img, graph_static, entry_s, side="static", speed=speed_s)
            elif vehicle_type_s == "bulldozer":
                new_vehicle_s = Bulldozer(bulldoz_img, graph_static, entry_s, side="static", speed=speed_s)
            elif vehicle_type_s == "ambulance":
                new_vehicle_s = Ambulance(ambulance_img, graph_static, entry_s, side="static", speed=speed_s)
            elif vehicle_type_s == "semitruck":
                new_vehicle_s = Semitruck(semi_img, graph_static, entry_s, side="static", speed=speed_s)
            else:
                new_vehicle_s = Policecar(policecar_img, graph_static, entry_s, side="static", speed=speed_s)

            vehicles_static.append(new_vehicle_s)

            # Create ML-side vehicle
            if vehicle_type_m == "car":
                new_vehicle_m = Car(cars_img, graph_ml, entry_m, side="ml", speed=speed_m)
            elif vehicle_type_m == "truck":
                new_vehicle_m = Truck(trucks_img, graph_ml, entry_m, side="ml", speed=speed_m)
            elif vehicle_type_m == "motorcycle":
                new_vehicle_m = Motorcycle(motos_img, graph_ml, entry_m, side="ml", speed=speed_m)
            elif vehicle_type_m == "schoolbus":
                new_vehicle_m = Schoolbus(schoolbus_img, graph_ml, entry_m, side="ml", speed=speed_m)
            elif vehicle_type_m == "bulldozer":
                new_vehicle_m = Bulldozer(bulldoz_img, graph_ml, entry_m, side="ml", speed=speed_m)
            elif vehicle_type_m == "ambulance":
                new_vehicle_m = Ambulance(ambulance_img, graph_ml, entry_m, side="ml", speed=speed_m)
            elif vehicle_type_m == "semitruck":
                new_vehicle_m = Semitruck(semi_img, graph_ml, entry_m, side="ml", speed=speed_m)
            else:
                new_vehicle_m = Policecar(policecar_img, graph_ml, entry_m, side="ml", speed=speed_m)

            vehicles_ml.append(new_vehicle_m)

######################################################################################################################
### CHANGES: Full list of Tiers, randomly chosen from your original code
def pick_traffic_conditions():
    # Tier 1: Time of Day => Spawn Rate
    tier1_options = {
        "Morning": {
    ...
    # Tier 2: Event => Weighted Probability of Vehicles
    tier2_events = {
        "ConstructionZone": {
    ...
    # Tier 3: Weather => Speed Overrides
    tier3_conditions = {
        "SnowStorm": {
    ...
    return tier1_info, tier2_info, tier3_info, chosen_tier1, chosen_tier2, chosen_tier3
...
def main():
    ...
    # ### (Optional) If you want consistent random picks, you can set random.seed(...) here
    ...
    pygame.display.set_caption("Traffic Simulation: No Traffic Signals (Random Conditions)")
    ...
    # Pick random Tiers
    tier1_info, tier2_info, tier3_info, label_t1, label_t2, label_t3 = pick_traffic_conditions()

    # Load images for Tiers
    ...
    ### CHANGES: We'll name the CSV as: "<Tier1>_<Tier2>_<Tier3>.csv"
    output_filename = f"Seed_{label_t1}_{label_t2}_{label_t3}.csv"
    ...
    spawner_thread = threading.Thread(
        ...
        args=(
            ...
            entry_static, entry_ml,
            graph_static, graph_ml,
            car_images, truck_images, motorcycle_images, schoolbus_images,
            policecar_images, bulldozer_images, ambulance_images, semitruck_images,
            tier1_info, tier2_info, tier3_info
        ),
        daemon=True
    )
    ...
    while running:
        ...
        # No traffic lights to update/draw
