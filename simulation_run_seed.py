...
parser.add_argument("--use-csv-seed", action="store_true", help="Use specific CSV seed setup")
parser.add_argument("--exit-when-done", action="store_true", help="Exit after all vehicles are processed")
parser.add_argument("--seed-file", type=str, help="Path to seed CSV file")
parser.add_argument("--ml-timings", type=str, help="Path to ML timing file", default="ml_timings.txt")
args = parser.parse_args()
###################################################################################################
def load_ml_nodes(graph_path):
    valid_nodes = set()
    with open(graph_path, newline='') as f:
        ...
        for row in reader:
            from_node = row["from"]
            to_node = row["to"]
            valid_nodes.add(from_node)
            valid_nodes.add(to_node)
    return valid_nodes
...
def load_graph_from_csv(csv_filename):#This function get the directed graph from the csv, the infomation of each node, and returns the graph, entry nodes, and exit nodes
    ...
    try:
        with open(csv_filename, "r", newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                if len(row) < 8 or row[1].strip().lower() == "node name":
                    continue
...
graph_ml, entry_nodes_ml, exit_nodes_ml = load_graph_from_csv("Directed_Graph - Machine_Learning.csv")
#########################################################################################################
...
############################################################################################################
class TrafficLight:
    def __init__(self, node_name, x, y, orientation, images, is_turning_lane=False, is_ml=False):
        ...
        self.is_ml = is_ml  # ML control flag
        self.base_cycle = 8  # Default cycle period
        self.cycle_period = self.base_cycle  # Will be overridden for ML lights

    def update(self):
        t = time.time() % self.cycle_period
        
        if self.is_ml:
            # ML-controlled light logic (simplified proportional control)
            green_time = 0.4 * self.cycle_period  # 40% green phase
            if t < green_time:
                self.state = "green"
            else:
                self.state = "red"
        else:
            # Original static light logic
            if self.orientation == "horizontal":
                if self.is_turning_lane:
                    if t < 1: self.state = "green"
                    else: self.state = "red"
                else:
                    if t < 1: self.state = "red"
                    elif t < 5: self.state = "green"
                    elif t < 6: self.state = "yellow"
                    else: self.state = "red"
            else:  # vertical
                if self.is_turning_lane:
                    if t < 5: self.state = "red"
                    elif t < 6: self.state = "green"
                    else: self.state = "red"
                else:
                    if t < 6: self.state = "red"
                    elif t < 7: self.state = "green"
                    elif t < 8: self.state = "yellow"
                    else: self.state = "red"
...
def create_traffic_lights(graph, horizontal_images, vertical_images, is_ml=False):
    traffic_lights = []
...
class Semitruck(BaseVehicle):
    ...
    def set_image_by_direction(self, direction):
        if direction in self.images:
            self.image = self.images[direction]
####################################################################################################################
def load_vehicle_seed_data(filename):
    vehicle_data = []
    try:
        with open(filename, newline='') as f:
            ...
            for row in reader:
                ...
                nodes = row.get("NodesSequence", "").split(";")
                if not nodes or not nodes[0].strip():
                    continue
                vehicle_data.append({
                    "VehicleType": row["VehicleType"],
                    "EntryNode": nodes[0],
                    "DelaySeconds": float(row["SpawnTime"]),
                    "Speed": float(row.get("Speed", 1.0))
                })
    except Exception as e:
        print(f"Failed to load seed CSV: {e}")
    return vehicle_data
...
def spawn_vehicles_thread(
    ...
    # --- Seed-based spawning ---
    if tier1_info.get("use_seed") and tier1_info.get("seed_data"):
        print("[DEBUG] Seeding vehicles from CSV...")
        seed_data = tier1_info["seed_data"]
        vehicle_classes = {
            "car": Car, "truck": Truck, "motorcycle": Motorcycle, "schoolbus": Schoolbus,
            "bulldozer": Bulldozer, "ambulance": Ambulance, "semitruck": Semitruck, "policecar": Policecar
        }
        image_lookup = {
            "car": cars_img, "truck": trucks_img, "motorcycle": motos_img, "schoolbus": schoolbus_img,
            "bulldozer": bulldoz_img, "ambulance": ambulance_img, "semitruck": semi_img, "policecar": policecar_img
        }

        # Sort seed data by spawn time
        seed_data.sort(key=lambda x: float(x["DelaySeconds"]))
        
        # Get the start time of the simulation
        start_time = time.time()
        next_spawn_index = 0
        
        while simulation_running and next_spawn_index < len(seed_data):
            current_time = time.time()
            elapsed = current_time - start_time
            
            # Check if it's time to spawn the next vehicle
            if elapsed >= float(seed_data[next_spawn_index]["DelaySeconds"]):
                row = seed_data[next_spawn_index]
                try:
                    vehicle_type = row["VehicleType"].strip().lower()
                    entry_node = row["EntryNode"]
                    speed = float(row["Speed"])

                    print(f"[DEBUG] Spawning {vehicle_type} at node {entry_node} (scheduled at {row['DelaySeconds']}s)")

                    # Validate node before spawning
                    if entry_node not in graph_ml or not graph_ml[entry_node]["neighbors"]:
                        print(f"[DEBUG] Skipping {vehicle_type} at {entry_node} — invalid node")
                        next_spawn_index += 1
                        continue

                    with lock:
                        cls = vehicle_classes.get(vehicle_type, Policecar)
                        img = image_lookup.get(vehicle_type, policecar_img)

                        v_ml = cls(img, graph_ml, entry_node, "ml", speed=speed, light_state=lights_state_ml)
                        vehicles_ml.append(v_ml)
                        print(f"[DEBUG] Appended {vehicle_type} to ML side at {elapsed:.2f}s")

                    next_spawn_index += 1

                except Exception as e:
                    print(f"Error spawning from seed: {e}")
                    next_spawn_index += 1
            
            # Small sleep to prevent busy waiting
            time.sleep(0.01)
            
        return  # exit after all seed vehicles spawned
...
def pick_traffic_conditions():
    ...
    tier2_events = {
        "Construction Zone": {
            ...
            "probabilities": {
                "car": 0.05, "truck": 0.05, "motorcycle": 0.05, "schoolbus": 0.05,
                "policecar": 0.05, "bulldozer": 0.45, "ambulance": 0.1, "semitruck": 0.2
            }
        },
        "Active Emergency Situation": {
            ...
            "probabilities": {
                "car": 0.075, "truck": 0.075, "motorcycle": 0.075, "schoolbus": 0.05,
                "policecar": 0.3, "bulldozer": 0.075, "ambulance": 0.3, "semitruck": 0.05
            }
        },
        "National Holiday": {
            "image_path": "images/Traffic_Conditions_images/Tier2_trafficConditions_NationalHoliday.png",
            "probabilities": {
                "car": 0.1, "truck": 0.06, "motorcycle": 0.06, "schoolbus": 0.06,
                "policecar": 0.1, "bulldozer": 0.06, "ambulance": 0.06, "semitruck": 0.5
            }
        },
        "School Commute Hours": {
            "image_path": "images/Traffic_Conditions_images/Tier2_trafficConditions_SchoolCommuteHours.png",
            "probabilities": {
                "car": 0.2, "truck": 0.1, "motorcycle": 0.075, "schoolbus": 0.4,
                "policecar": 0.08, "bulldozer": 0.04, "ambulance": 0.075, "semitruck": 0.03
            }
        },
        "Work Rush Hours": {
            "image_path": "images/Traffic_Conditions_images/Tier2_trafficConditions_WorkRushHours.png",
            "probabilities": {
                "car": 0.25, "truck": 0.25, "motorcycle": 0.25, "schoolbus": 0.05,
                "policecar": 0.05, "bulldozer": 0.05, "ambulance": 0.05, "semitruck": 0.05
            }
        }
    }
    ...
    tier3_conditions = {
        "SnowStorm": {
            ...
            "speeds": {
                "car": 1.0, "truck": 0.7, "motorcycle": 1.25, "schoolbus": 0.8,
                "policecar": 1, "bulldozer": 0.5, "ambulance": 1, "semitruck": 0.6
            }
        },
        "ClearSkies": {
            "image_path": "images/Traffic_Conditions_images/Tier3_trafficConditions_ClearSkies.png",
            "speeds": {
                "car": 1.5, "truck": 1, "motorcycle": 1.75, "schoolbus": 1,
                "policecar": 1.5, "bulldozer": 0.8, "ambulance": 1.25, "semitruck": 0.75
            }
        },
        "ExtremeHeat": {
            "image_path": "images/Traffic_Conditions_images/Tier3_trafficConditions_ExtremeHeat.png",
            "speeds": {
                "car": 2.0, "truck": 2.8, "motorcycle": 2.8, "schoolbus": 1.7,
                "policecar": 2.0, "bulldozer": 1.5, "ambulance": 2.9, "semitruck": 1.7
            }
        },
        "ModerateWindGusts": {
            "image_path": "images/Traffic_Conditions_images/Tier3_trafficConditions_ModerateWindGusts.png",
            "speeds": {
                "car": 1.65, "truck": 1.15, "motorcycle": 2, "schoolbus": 1.15,
                "policecar": 1.65, "bulldozer": 1, "ambulance": 1.5, "semitruck": 0.9
            }
        },
        "Thunderstorm": {
            "image_path": "images/Traffic_Conditions_images/Tier3_trafficConditions_Thunderstorm.png",
            "speeds": {
                "car": 0.85, "truck": 0.65, "motorcycle": 1, "schoolbus": 0.7,
                "policecar": 0.8, "bulldozer": 0.5, "ambulance": 0.7, "semitruck": 0.4
            }
        }
    }

    if args.use_csv_seed and args.seed_file:
        seed_data = load_vehicle_seed_data(args.seed_file)
        if not seed_data:
            raise ValueError("Seed data is empty or invalid.")

        filename = os.path.basename(args.seed_file).lower()

        time_of_day = next((k for k in ["morning", "afternoon", "evening", "night"] if k in filename), "morning").capitalize()
        event_type = next((k for k in tier2_events if k.replace(" ", "_").lower() in filename.replace("_", " ")), "Work Rush Hours")
        weather_type = next((k for k in tier3_conditions if k.lower() in filename), "ClearSkies")

        tier1_info = tier1_options[time_of_day]
        tier1_info["use_seed"] = True
        tier1_info["seed_data"] = seed_data
        tier1_info["spawn_min"] = 0
        tier1_info["spawn_max"] = 0

        tier2_info = tier2_events[event_type]
        tier3_info = tier3_conditions[weather_type]

    else:
        # default fallback: random
        tier1_key = random.choice(list(tier1_options.keys()))
        tier2_key = random.choice(list(tier2_events.keys()))
        tier3_key = random.choice(list(tier3_conditions.keys()))

        tier1_info = tier1_options[tier1_key]
        tier2_info = tier2_events[tier2_key]
        tier3_info = tier3_conditions[tier3_key]
...
def main():
    ...
    def load_ml_timings():
        """Load ML-generated timing data from file"""
        try:
            with open(args.ml_timings, "r") as f:
                return [float(line.strip()) for line in f]
        except FileNotFoundError:
            print(f"⚠️ No ML timings found at {args.ml_timings} - using default cycles")
            return None
    ...
    lights_state_ml = {tl.node_name: tl for tl in ml_traffic_lights}
        # Apply ML timings
    ml_timings = load_ml_timings()
    if ml_timings and len(ml_timings) == len(ml_traffic_lights):
        for tl, timing in zip(ml_traffic_lights, ml_timings):
            tl.cycle_period = timing
            print(f"🚦 ML Light {tl.node_name} cycle set to {timing:.1f}s")
    else:
        print("⚠️ Using default cycles for ML lights")

        
    tier1_info, tier2_info, tier3_info = pick_traffic_conditions()
    seed_data = None
    total_expected_vehicles = None
